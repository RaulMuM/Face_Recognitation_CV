...
while True:
    ...
    for facecod, faceloc in zip(facescod, faces):
        ...
        if comparacion[min]:
            ...
            if comp1 == indice:
                # Dibujamos Latest cheer with Access Permitted
                cv2.rectangle (frame, (xi, yi), (xf, yf), (r, g, b), 3)
                cv2.rectangle(frame, (xi, yf-35), (xf, yf), (r, g, b), cv2.FILLED)
                cv2.putText(frame, nombre + ": ACCESO PERMITIDO", (xi+6, yf-6), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2)
            
                horario(nombre)
        else:
            # Dibujamos Latest cheer: Access Denied
            cv2.rectangle (frame, (xi, yi), (xf, yf), (0, 0, 255), 3)
            cv2.rectangle(frame, (xi, yf-35), (xf, yf), (0, 0, 255), cv2.FILLED)
            cv2.putText(frame, "ACCESO DENEGADO", (xi+6, yf-6), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2)
